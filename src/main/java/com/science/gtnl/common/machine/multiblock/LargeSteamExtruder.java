...
public class LargeSteamExtruder extends MTESteamMultiBase<LargeSteamExtruder> implements ISurvivalConstructable {
    ...
    public static int getTierMaterialBlockCasing(Block block, int meta) {
    ...
    public int getCasingTextureID() {
        if (tierGearCasing == 2 || tierMachineCasing == 2
            ...
            || tierMaterialBlock == 2
            || tierBrickCasing == 2
            || tierPipeCasing == 2) return ((BlockCasings2) GregTechAPI.sBlockCasings2).getTextureIndex(0);
    ...
    protected GTRenderedTexture getFrontOverlay() {
        return new GTRenderedTexture(oMCDIndustrialExtruder);
    }

    @Override
    protected GTRenderedTexture getFrontOverlayActive() {
        return new GTRenderedTexture(oMCDIndustrialExtruderActive);
    }
        ...
        if (STRUCTURE_DEFINITION == null) {
            STRUCTURE_DEFINITION = StructureDefinition.<LargeSteamExtruder>builder()
                ...
                .addElement(
                    'A',
                    ofChain(
                        buildSteamInput(LargeSteamExtruder.class)
    ...
    public Collection<RecipeMap<?>> getAvailableRecipeMaps() {
        return Arrays.asList(RecipeMaps.extruderRecipes);
    }
