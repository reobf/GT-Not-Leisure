...
public class BlazePickaxe extends ItemPickaxe {
    ...
    public void addInformation(final ItemStack itemStack, final EntityPlayer player, final List toolTip,
        ...
        toolTip.add(
            StatCollector.translateToLocal(
                isSmeltingModeActive(itemStack) ? TextLocalization.Tooltip_BlazePickaxe_Smelt_On
                    : TextLocalization.Tooltip_BlazePickaxe_Smelt_Off));
    }
    ...
    public void onCreated(ItemStack stack, World world, EntityPlayer player) {
        ...
        updateEnchantments(stack, false);
    }
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (player.isSneaking()) {
            ...
            String messageKey = isSmeltingModeActive(stack) ? TextLocalization.Tooltip_BlazePickaxe_Smelt_On
                : TextLocalization.Tooltip_BlazePickaxe_Smelt_Off;
            if (world.isRemote) {
    ...
    private void toggleSmeltingMode(ItemStack stack) {
        ...
        updateEnchantments(stack, !currentMode);
    }

    private void updateEnchantments(ItemStack stack, boolean smeltingActive) {
        Map<Integer, Integer> enchantments = EnchantmentHelper.getEnchantments(stack);
        if (smeltingActive) {
            enchantments.put(Enchantment.fireAspect.effectId, 10);
        } else {
            enchantments.remove(Enchantment.fireAspect.effectId);
        }
